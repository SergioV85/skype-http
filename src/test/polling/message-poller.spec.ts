...
describe('formatControlClearTypingResource', function () {
  ...
  const items: Item[] = [
    {
      nativeResource: {
        ...
        ackrequired:
          'https://db5-client-s.gateway.messenger.live.com/v1/users/ME/conversations/ALL/messages/1483879804631/ack',
      ...
      expectedFormattedResource: {
        ...
        // tslint:disable-next-line:max-line-length
        conversation: messagesUri.parseConversation(
          'https://db5-client-s.gateway.messenger.live.com/v1/users/ME/conversations/8:bob',
        ).conversation,
        native: {
          id: '1483879804631',
          // tslint:disable-next-line:max-line-length
  ...
  for (const item of items) {
    it('should format a Control/ClearTyping Message resource', function () {
      // tslint:disable-next-line:max-line-length
      const actual: resources.ControlClearTypingResource = formatControlClearTypingResource(
        formatGenericMessageResource(item.nativeResource, item.nativeResource.messagetype),
        item.nativeResource,
      );
...
describe('formatControlTypingResource', function () {
  ...
  const items: Item[] = [
    {
      nativeResource: {
        ...
        // tslint:disable-next-line:max-line-length
        ackrequired:
          'https://db5-client-s.gateway.messenger.live.com/v1/users/ME/conversations/ALL/messages/1483885996187/ack',
  ...
  for (const item of items) {
    it('should format a Control/Typing Message resource', function () {
      // tslint:disable-next-line:max-line-length
      const actual: resources.ControlTypingResource = formatControlTypingResource(
        formatGenericMessageResource(item.nativeResource, item.nativeResource.messagetype),
        item.nativeResource,
      );
